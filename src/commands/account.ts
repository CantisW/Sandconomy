...
export class Account {
        ...
        if (!bal) {
            if (await CheckIfAccountExists(interaction.user.id)) {
                ...
                const embed = new MessageEmbed()
                    ...
                    {
                        "name": "Experience",
                        "value": `Currently at level ${level} with ${xp} Experience Points\n${await xpUntilNextLevel(interaction.user.id) - xp} points until next level`
                    },
                    {
                        "name": "Times Worked",
        ...
        if (await CheckIfAccountExists(bal.id)) {
            const { balance, cash, bank, xp, level, timesWorked } = await GetAccountInfo(bal.id);
                const embed = new MessageEmbed()
                    ...
                    {
                        "name": "Experience",
                        "value": `Currently at level ${level} with ${xp} Experience Points\n${await xpUntilNextLevel(bal.id) - xp} points until next level`
                    },
                    {
                        "name": "Times Worked",
    ...
    async deposit (
        @SlashOption("amount", { "description": "How much? [ number or leave blank to deposit all ]", "required": false, "type":"NUMBER" })
        amount: number,
        interaction: CommandInteraction
    ) {
        if (await CheckIfAccountExists(interaction.user.id)) {
